...
void testcase() {
    ...
    while(!to_visit.empty()) {
        ...
        if (f->info() < d) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if(delaunay.is_infinite(nf)) continue;
    ...
    for(int i = 0; i < m; i++) {
        ...
        K::FT required = CGAL::square(K::FT(s + r));
        K::FT dist = CGAL::squared_distance(b, v);
        if(dist >= 4 * required) {
            std::cout << "y";
            continue;
        }
        ...
        if(delaunay.locate(b)->info() >= 4 * required) {
            std::cout << "y";
            continue;
        }
        std::cout << "n";
